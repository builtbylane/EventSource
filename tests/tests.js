...
(function (global) {
  ...
  var mainTests = function () {
    ...
    asyncTest(
      'EventSource: lastEventId should not be set when connection dropped without data dispatch',
      function () {
        var body =
          'retry: 500\n' +
          'data: -1\n\n' +
        ...
        es.onmessage = function (event) {
          if (Number(event.lastEventId) === 2) {
            ...
            ok(
              false,
              'lastEventId should not be set when connection dropped without data dispatch (see http://www.w3.org/Bugs/Public/show_bug.cgi?id=13761 )'
            );
            start();
          }
        };
    ...
    asyncTest(
      'EventSource: Once the end of the file is reached, any pending data must be discarded',
      function () {
        var body = 'data: data0';
        var es = new EventSource(
          url + '?estest=' + encodeURIComponent(commonHeaders + '\n\n' + body)
    ...
    asyncTest('unicode', function () {
      var body =
        'data:<byte(F0)><delay(500)><byte(9F)><delay(500)><byte(8F)><delay(500)><byte(A9)><delay(500)>\n\n';
      var es = new EventSource(
        url + '?estest=' + encodeURIComponent(commonHeaders + '\n\n' + body)
      );
      es.onmessage = function (event) {
    ...
    asyncTest('EventSource + close before open', function () {
      var es = new EventSource(
        url +
          '?estest=' +
          encodeURIComponent(commonHeaders + '\n\n' + 'data:test\n\n')
      );
      var events = '';
  ...
  var otherTests = function () {
    ...
    asyncTest(
      'URL with username and password for Basic Authorization',
      function () {
        var es = undefined;
        try {
          es = new EventSource(
            urlWithAuthorization +
              '?authorization=true&estest=' +
              encodeURIComponent(
                commonHeaders + '\n\n' + '\ndata:<authorization>\n\n'
              )
          );
    ...
    asyncTest('blob: URL', function () {
      if (global.URL != undefined) {
        ...
        es.onerror = function (event) {
          ok(
            message === '1' && es.readyState === EventSource.CONNECTING,
            'failed'
          );
          es.close();
          start();
    ...
    asyncTest('data: URL', function () {
      ...
      try {
        es = new EventSource(
          'data:text/event-stream;charset=utf-8,' +
            encodeURIComponent('retry:1000\ndata:1\n\n')
        );
      } catch (error) {
        if (global.console != undefined) {
          global.console.log(error);
        ...
        url +
          ...
          encodeURIComponent(
            commonHeaders +
              ...
              new Date(new Date().getTime() + 3600000).toUTCString() +
              '\n\n' +
              'retry:1000\ndata:<random()>\n\n'
          )
      );
      ...
      setTimeout(function () {
        ...
        ok(
          es.readyState === EventSource.CLOSED
            ? !isActive &&
                lastEventType === 'error' &&
                readyStateAtLastEvent === EventSource.CLOSED
            : isActive,
          ''
        );
        es.close();
    ...
    asyncTest(
      'EventSource retry delay - see http://code.google.com/p/chromium/issues/detail?id=86230',
      function () {
        var body = 'retry: 800\n\n';
        var es = new EventSource(
          url + '?estest=' + encodeURIComponent(commonHeaders + '\n\n' + body)
    ...
    asyncTest(
      'EventSource: event-stream with "message", "error", "open" events',
      function () {
        var body =
          'data: a\n\n' +
          'event: open\ndata: b\n\n' +
    ...
    asyncTest('infinite reconnection', function () {
      var es = new EventSource(
        'http://functionfunction' + Math.floor(Math.random() * 1e10) + '.org'
      );
      var n = 0;
      es.onerror = function () {
        ++n;
